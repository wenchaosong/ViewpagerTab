...
class SmartTabStrip extends LinearLayout {
    ...
    private final boolean indicatorIsTrig;
    private final int indicatorTrigWidth;
    private final int indicatorTrigHeight;

    private Paint mTrigPaint;
    private Path mTrigPath = new Path();
    ...
    SmartTabStrip(Context context, AttributeSet attrs) {
        ...
        indicatorIsTrig = a.getBoolean(
                R.styleable.stl_SmartTabLayout_stl_indicatorTrig, false);
        indicatorTrigWidth = a.getLayoutDimension(
                R.styleable.stl_SmartTabLayout_stl_indicatorTrigWidth, Utils.dip2px(context, 14));
        indicatorTrigHeight = a.getLayoutDimension(
                R.styleable.stl_SmartTabLayout_stl_indicatorTrigHeight, Utils.dip2px(context, 8));
        a.recycle();
        ...
        this.mTrigPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness,
        ...
        if (indicatorIsTrig) {
            drawTrig(canvas, left, right, color);
        }
    }

    private void drawTrig(Canvas canvas, int left, int right, int color) {
        mTrigPaint.setColor(color);
        mTrigPath.reset();
        mTrigPath.moveTo(left + (right - left) / 2 - (indicatorTrigWidth / 2), getHeight());
        mTrigPath.lineTo(left + (right - left) / 2, getHeight() - indicatorTrigHeight);
        mTrigPath.lineTo(left + (right - left) / 2 + (indicatorTrigWidth / 2), getHeight());
        mTrigPath.close();
        canvas.drawPath(mTrigPath, mTrigPaint);
    }
